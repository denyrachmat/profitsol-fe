<template>
  <div class="q-pa-md bg-grey">
    ...
    <div v-else>
      <!-- This is marker for check answer / not -->
            ...
            <div class="col">
              <div class="row">
                <div
                  ...
                  v-for="idx in getFormsOnly.length"
                  :key="idx"
                >
                  <div
...
onMounted(() => {
  ...
  if (props.data.length > 0) {
    ...
    console.log(getNowQuestion.value);
    console.log(checkAnySameHTML.value);
  }
});
...
const getNowQuestion = computed(() => {
  if (getNowData.value) {
    let checkDataForm = datanya.value.filter(
      (fil) => fil.type === "form" && fil.seq_name == getNowData.value.seq_name
    );

    return checkDataForm.length > 1
      ? checkDataForm[formSeq.value]
      : checkDataForm[0];
  }
...
const getNowData = computed(() => datanya.value[nowSeq.value]);
...
const getFormsOnly = computed(() =>
  datanya.value.filter((fil) => fil.type === "form")
);
...
const checkAnySameHTML = computed(() => {
  if (getNowData.value) {
    return Object.values(
      props.data.filter(
        (val) => val.type == "html" && val.seq_name == getNowData.value.seq_name
      )
    );
  }
